...
def eval_params_on_ticker(args):
    tkr, df, nf, ns = args
    res_df, _ = run_walk_forward_fixed(df, n_fast=nf, n_slow=ns)
...
def main():
    ...
    tasks = [(t, price_cache_in[t], nf, ns) for t in learn_list for (nf,ns) in cand]
    ...
    if not scored:
        raise RuntimeError("No scores computed on learning set.")
    ...
    def eval_group(tickers, label, use_holdout=False):
        ...
        for t in tickers:
            ...
            if df is None or df.empty:
                ...
                continue
            res_df, eq = run_walk_forward_fixed(df, n_fast=best_nf, n_slow=best_ns)
            save_outputs(f"{t}_{label}", res_df, eq)
    ...
    out.to_csv("reports/_all_summary.csv", index=False)
    with open("reports/_params.txt","w",encoding="utf-8") as f:
        f.write(f"best_n_fast={best_nf}\nbest_n_slow={best_ns}\nscore={best_score:.6f}\n")
        f.write(f"learn_nonAI={learn_list}\nOOS_fixed={fixed}\nOOS_random={rand_oos}\n")
        f.write(f"holdout_months={HOLDOUT_MONTHS}\n")

    print("Backtest done. Reports in ./reports")
